...
# USAGE
#
#   tools/ci/clean_room.sh                 # fast: --no-deps, proves the CLI works without torch
#   tools/ci/clean_room.sh --full          # the real clean-machine install, torch and all. Slow.
#   tools/ci/clean_room.sh --host HOST     # run it on another machine over ssh
#
set -euo pipefail
...
cp "$WHEEL" "$CHECK_WHEEL" "$ROOT/tools/ci/clean_room_checks.py" "$WORK/"
