...
#
#     ln -s ../../tools/hooks/precommit-prompt-artefacts.sh .githooks/local.d/10-prompt-artefacts.sh
#
...
# reviewable, with only the wiring outside. `tools/hooks/install-local-hooks.sh` makes the link.
#
...
for py in python3 python; do
    if command -v "$py" >/dev/null 2>&1; then
        exec "$py" tools/ci/check_prompt_artefacts.py --staged
    fi
done
