...
#
# Usage:  tools/dev/record-live.sh <demo-name> [target-width]
...
OUT="$MEDIA/$NAME.gif"
CAST="/usr/bin/python3 $ROOT/tools/dev/_screencast.py"
...
[ -n "$NAME" ] || { echo "usage: tools/dev/record-live.sh <demo-name> [target-width]" >&2; exit 2; }
...
if ! BBOX="$("$VENV_PY" "$ROOT/tools/dev/_screencast.py" bbox "$STATE/probe.png" "$SENBON_LIVE_SENTINEL" 2>&1)"; then
    ...
    echo "    SENBON_LIVE_MONITOR=<connector> tools/dev/record-live.sh $NAME" >&2
    exit 4
fi
